...
WIFI_MODULES:
	rm -rf $(KERNEL_OUT)/$(TARGET_MODULES_SOURCE_DIR)
	cp -rf $(TARGET_MODULES_SOURCE) $(KERNEL_OUT)
	$(MAKE) -C $(KERNEL_OUT)/$(TARGET_MODULES_SOURCE_DIR) O=$(KERNEL_OUT)/COMPAT KERNEL_DIR=$(KERNEL_OUT) KLIB=$(KERNEL_OUT) KLIB_BUILD=$(KERNEL_OUT) ARCH=$(TARGET_ARCH) $(KERNEL_CROSS_COMPILE)
	mv $(KERNEL_OUT)/$(TARGET_MODULES_SOURCE_DIR)/{compat/compat,net/mac80211/mac80211,net/wireless/cfg80211,drivers/net/wireless/wl12xx/wl12xx,drivers/net/wireless/wl12xx/wl12xx_sdio}.ko $(KERNEL_MODULES_OUT)
	$(KERNEL_TOOLCHAIN_PATH)strip --strip-unneeded $(KERNEL_MODULES_OUT)/{compat,cfg80211,mac80211,wl12xx,wl12xx_sdio}.ko
