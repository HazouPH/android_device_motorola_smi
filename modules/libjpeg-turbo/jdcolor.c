         ...
         JSAMPARRAY output_buf, int num_rows)
{
  ...
  while (--num_rows >= 0) {
    ...
    for (col = 0; col < num_cols; col++) {
      ...
      outptr[EXT_RGBX_RED] =   range_limit[y + Crrtab[cr]];
      outptr[EXT_RGBX_GREEN] = range_limit[y +
                              ((int) RIGHT_SHIFT(Cbgtab[cb] + Crgtab[cr],
                                                 SCALEBITS))];
      outptr[EXT_RGBX_BLUE] =  range_limit[y + Cbbtab[cb]];
      outptr[3] =  0xFF;
      outptr += 4;
    }
  }
          ...
          JSAMPARRAY output_buf, int num_rows)
{
  ...
  while (--num_rows >= 0) {
    ...
    for (col = 0; col < num_cols; col++) {
      ...
      outptr[EXT_RGBX_RED] = outptr[EXT_RGBX_GREEN] = outptr[EXT_RGBX_BLUE] = inptr[col];
      outptr[3] = 0xff;
      outptr += 4;
    }
  }
...
jinit_color_deconverter (j_decompress_ptr cinfo)
{
  ...
  case JCS_EXT_ARGB:
    ...
    if (cinfo->jpeg_color_space == JCS_YCbCr) {
      ...
      ERREXIT(cinfo, JERR_CONVERSION_NOTIMPL);
    break;
#if 0 // currently we use JCS_EXT_RGBA for JCS_RGBA_8888
#ifdef ANDROID_RGB
  case JCS_RGBA_8888:
