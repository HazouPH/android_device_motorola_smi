...
*/
//int32_t ALACEngine::Init( void * inMagicCookie, uint32_t inMagicCookieSize )
int32_t ALACEngine::Init( ALACSpecificConfig *pALACConfig )
{
...
//    uint32_t theCookieBytesRemaining = inMagicCookieSize;
	...
	// Notes on MP4a
	//
	// Current iTunes version (v10.4.1.10) generates "legacy" Magic cookie, including 'frma' + 'alac' chunks
	// as noted in Apple's reference code document "ALACMagicCookieDescription.txt'
	// Total length = 56 bytes.  
	// 0-23:	frma + alac + legnth info
	// 24-47:	Magic Cookie (24 bytes)
	// 48-55:   termination atom = Channel Layout Info Size = 8 (4 bytes) + Channel Layout Info ID = 0 (4 bytes) i.e., both uint32_t
...
/*
	...
	- the decoded samples are interleaved into the output buffer in the order they arrive in
	  the bitstream
*/
int32_t ALACEngine::Decode( BitBuffer * bits, uint8_t * sampleBuffer, uint32_t numSamples, uint32_t numChannels, uint32_t * outNumSamples )
{
	BitBuffer			shiftBits;
...
*/
int32_t ALACEngine::FillElement( BitBuffer * bits )
{
	int16_t		count;
...
*/
int32_t ALACEngine::DataStreamElement( BitBuffer * bits )
{
