...
struct module *module_open(const char *file, int flag)
{
    ...
    if (existing) {
        ALOGE("found opened module %s with name\n", existing->name);
        existing->ref_count++;
    ...
    if (existing) {
        ALOGE("found opened module %s with handle\n", existing->name);
        existing->ref_count++;
    ...
    dlerr = dlerror();
    if (!dlerr && new->init) {
        ALOGE("module %s has init(), call the symbol\n", new->name);
        init_ret = new->init(new);
    }

    if (init_ret) {
        ALOGE("module %s init() failed (%d)\n", new->name, init_ret);
        goto free_handle;
    }
...
int module_close(struct module *module)
{
    ...
    ALOGV("module %s decrease refcont (%d)\n", module->name, module->ref_count);
    ...
    if (!module->ref_count) {
        ...
        if (dlclose(module->handle)) {
            ALOGE("module %s dlclose failed",module->name);
        }

        g_module_head = module_del_list(g_module_head, module);

        ALOGV("module %s closed\n", module->name);
...
void *module_symbol(struct module *module, const char *string)
{
    ...
    if (!symbol) {
        dlerr = dlerror();
        ALOGE("not founded symbol %s in module %s (%s)\n",
             string, module->name, dlerr);
        module_set_error(dlerr);
        symbol = NULL;
    }
    else
        ALOGV("found symbol %s in module %s\n", string, module->name);
