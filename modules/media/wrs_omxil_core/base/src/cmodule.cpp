...
OMX_ERRORTYPE CModule::Load(int flag)
{
    ...
    if (!wrs_omxil_cmodule) {
        ALOGE("module %s symbol not founded (%s)\n",
    ...
    if (module)
        ALOGE("module %s will be overwrite",module->name);
    module = m;
    ALOGI("module %s successfully loaded\n", lname);
...
OMX_U32 CModule::Unload(void)
{
    ...
    if (!ref_count) {
        ...
        ALOGI("module %s successfully unloaded\n", lname);
    }
