...
OMXVideoEncoderH263::OMXVideoEncoderH263() {
    ALOGV("Constructer for OMXVideoEncoderH263.");
    BuildHandlerList();
    mVideoEncoder = createVideoEncoder(H263_MIME_TYPE);
    if (!mVideoEncoder) ALOGE("OMX_ErrorInsufficientResources");
#ifdef SYNC_MODE
...
OMXVideoEncoderH263::~OMXVideoEncoderH263() {
    ALOGV("Destructer for OMXVideoEncoderH263.");
}
...
OMX_ERRORTYPE OMXVideoEncoderH263::ProcessorInit(void) {
    ALOGV("OMXVideoEncoderH263::ProcessorInit\n");
...
OMX_ERRORTYPE OMXVideoEncoderH263::ProcessorProcess(
    ...
    ALOGV("OMXVideoEncoderH263::ProcessorProcess \n");
    ...
    ALOGV("%s(): enter encode\n", __func__);

    ALOGV_IF(buffers[INPORT_INDEX]->nFlags & OMX_BUFFERFLAG_EOS,
            "%s(),%d: got OMX_BUFFERFLAG_EOS\n", __func__, __LINE__);

    if (!buffers[INPORT_INDEX]->nFilledLen) {
        ALOGE("%s(),%d: input buffer's nFilledLen is zero\n", __func__, __LINE__);
        goto out;
    }
    ...
    ALOGV("buffer_in.data=%x, data_size=%d",
    ...
    ALOGV("output data size = %d", outBuf.dataSize);
    ...
    if(ret == ENCODE_SLICESIZE_OVERFLOW) {
        ALOGV("%s(), mix_video_encode returns MIX_RESULT_VIDEO_ENC_SLICESIZE_OVERFLOW"
             , __func__);
...
out:
    ...
    if(retains[OUTPORT_INDEX] != BUFFER_RETAIN_GETAGAIN) {
        ...
        ALOGV("********** output buffer: len=%d, ts=%ld, flags=%x",
             outfilledlen,
             outtimestamp,
             outflags);
    }
    ...
    ALOGV_IF(oret == OMX_ErrorNone, "%s(),%d: exit, encode is done\n", __func__, __LINE__);
...
OMX_ERRORTYPE OMXVideoEncoderH263::SetParamVideoProfileLevelQuerySupported(OMX_PTR pStructure) {
    ALOGW("SetParamVideoH263ProfileLevel is not supported.");
