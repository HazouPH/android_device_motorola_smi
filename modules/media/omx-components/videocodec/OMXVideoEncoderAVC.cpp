...
OMXVideoEncoderAVC::OMXVideoEncoderAVC() {
    ...
    if (!mVideoEncoder) {
        ALOGE("OMX_ErrorInsufficientResources");
        return;
    }
...
OMX_ERRORTYPE OMXVideoEncoderAVC::InitOutputPortFormatSpecific(OMX_PARAM_PORTDEFINITIONTYPE *paramPortDefinitionOutput) {
    ...
    if (mPLTableCount > 0) {
        ...
        ALOGE("No supported profile/level\n");
...
OMX_ERRORTYPE OMXVideoEncoderAVC::SetVideoEncoderParam(void) {
    ...
    ALOGV("OMXVideoEncoderAVC::SetVideoEncoderParam");

    if (!mEncoderParams) {
        ALOGE("NULL pointer: mEncoderParams");
...
OMX_ERRORTYPE OMXVideoEncoderAVC::ProcessorPreEmptyBuffer(OMX_BUFFERHEADERTYPE* buffer) {
    ...
    ALOGV("ProcessorPreEmptyBuffer idrPeriod=%d, IntraPeriod=%d, IpPeriod=%d, BFrameEnabled=%d\n", idrPeriod, IntraPeriod, IpPeriod, BFrameEnabled);
...
OMX_BOOL OMXVideoEncoderAVC::ProcessCacheOperation(OMX_BUFFERHEADERTYPE **buffers) {
    ...
    ALOGV("ProcessCacheOperation Frame %d, type:%s, CacheOps:%s, NoSTOP=%d, EOS=%d\n",
    ...
    if (buffers[INPORT_INDEX]->nFilledLen == 0 && buffers[INPORT_INDEX]->nFlags & OMX_BUFFERFLAG_EOS) {
        ...
        ALOGV("ProcessCacheOperation: This frame is Empty EOS buffer\n");
    }
    ...
    if (eInfo.CacheOperation == CACHE_NONE) {
        ...
        Cached = OMX_TRUE;
        ALOGV("ProcessCacheOperation: This B frame is cached\n");
...
//        mBFrameList.clear();
    ...
    */
    ...
    ALOGV("ProcessCacheOperation Completed return %d\n", Cached);
    return Cached;
}
...
OMX_ERRORTYPE OMXVideoEncoderAVC::ProcessDataRetrieve(
    ...
    switch (NaluFormat) {
        ...
        case OMX_NaluFormatLengthPrefixedSeparateFirstHeader:
            if(!mCSDOutputted) {
                ALOGV("Output codec data for first frame\n");
    ...
    if (ret < ENCODE_SUCCESS) {
        ALOGE("libMIX getOutput Failed. ret = 0x%08x\n", ret);
        outBuf.dataSize = 0;
        ...
        if (ret == ENCODE_NO_REQUEST_DATA) {
            if (mEmptyEOSBuf) {
                ...
                ALOGV("no more data encoding, will signal empty EOS output buf\n");
            } else {
                //if not meet Empty EOS buffer, shouldn't get this error
                ALOGE("sever error, should not happend here\n");
        ...
        ALOGV("Call libMIX getOutput againe due to 'data not ready'\n");
        ret = mVideoEncoder->getOutput(&outBuf);
    }

    ALOGV("libMIX getOutput data size= %d, flag=0x%08x", outBuf.dataSize, outBuf.flag);
    ...
    if(outBuf.flag & ENCODE_BUFFERFLAG_ENDOFFRAME) {
        ALOGV("got a complete libmix Frame\n");
    ...
    if (outfilledlen == 0) {
        if (mEmptyEOSBuf) {
            ...
            *outBufReturned = OMX_TRUE;
            ALOGV("emit one empty EOS OMX output buf = %p:%d, flag = 0x%08x, ts=%lld", buffers[OUTPORT_INDEX]->pBuffer, outfilledlen, outflags, outtimestamp);
        } else
        ...
        *outBufReturned = OMX_TRUE;
        ALOGV("emit one OMX output buf = %p:%d, flag = 0x%08x, ts=%lld", buffers[OUTPORT_INDEX]->pBuffer, outfilledlen, outflags, outtimestamp);

    }

    ALOGV("ProcessDataRetrieve OK, mFrameEncodedCount=%d , mFrameOutputCount=%d\n", mFrameEncodedCount, mFrameOutputCount);
    return OMX_ErrorNone;
}
...
OMX_ERRORTYPE OMXVideoEncoderAVC::ProcessorProcess(
    ...
    if (buffers[INPORT_INDEX]) {
        ALOGV("input buffer has new frame\n");
    ...
    ALOGV("ProcessorProcess ret=%x", oret);
    return oret;
...
OMX_ERRORTYPE OMXVideoEncoderAVC::SetParamNalStreamFormatSupported(OMX_PTR pStructure) {
    ALOGW("SetParamNalStreamFormatSupported is not supported.");
    return OMX_ErrorUnsupportedSetting;
}

OMX_ERRORTYPE OMXVideoEncoderAVC::GetParamNalStreamFormatSelect(OMX_PTR pStructure) {
    ALOGW("GetParamNalStreamFormatSelect is not supported.");
    return OMX_ErrorUnsupportedSetting;
}
...
OMX_ERRORTYPE OMXVideoEncoderAVC::SetConfigVideoAVCIntraPeriod(OMX_PTR pStructure) {
    ...
    if(retStatus !=  ENCODE_SUCCESS) {
        ALOGW("set avc intra period config failed");
    }
...
OMX_ERRORTYPE OMXVideoEncoderAVC::SetConfigVideoNalSize(OMX_PTR pStructure) {
    ...
    if (mParamBitrate.eControlRate == OMX_Video_ControlRateMax) {
        ALOGE("SetConfigVideoNalSize failed. Feature is disabled.");
    ...
    if (mParamBitrate.eControlRate != OMX_Video_Intel_ControlRateVideoConferencingMode) {
        ALOGE("SetConfigVideoNalSize failed. Feature is supported only in VCM.");
        return OMX_ErrorUnsupportedSetting;
    }
    ...
    if(retStatus != ENCODE_SUCCESS) {
        ALOGW("set NAL size config failed");
    }
    return OMX_ErrorNone;
}
...
OMX_ERRORTYPE OMXVideoEncoderAVC::SetConfigIntelSliceNumbers(OMX_PTR pStructure) {
    ...
    Encode_Status retStatus = ENCODE_SUCCESS;
    if (mParamBitrate.eControlRate == OMX_Video_ControlRateMax) {
        ALOGE("SetConfigIntelSliceNumbers failed. Feature is disabled.");
        return OMX_ErrorUnsupportedIndex;
    }
    OMX_VIDEO_CONFIG_INTEL_SLICE_NUMBERS *p = (OMX_VIDEO_CONFIG_INTEL_SLICE_NUMBERS *)pStructure;
    CHECK_TYPE_HEADER(p);
    ...
    if (mParamBitrate.eControlRate != OMX_Video_Intel_ControlRateVideoConferencingMode) {
        ALOGE("SetConfigIntelSliceNumbers failed. Feature is supported only in VCM.");
        return OMX_ErrorUnsupportedSetting;
    }
    ...
    if(retStatus != ENCODE_SUCCESS) {
        ALOGW("set silce num config failed!\n");
    }
    return OMX_ErrorNone;
}
