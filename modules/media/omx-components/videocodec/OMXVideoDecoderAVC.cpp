...
OMXVideoDecoderAVC::OMXVideoDecoderAVC()
    ...
    ALOGV("OMXVideoDecoderAVC is constructed.");
    mVideoDecoder = createVideoDecoder(AVC_MIME_TYPE);
    if (!mVideoDecoder) {
        ALOGE("createVideoDecoder failed for \"%s\"", AVC_MIME_TYPE);
    }
...
OMXVideoDecoderAVC::~OMXVideoDecoderAVC() {
    ALOGV("OMXVideoDecoderAVC is destructed.");
}
...
OMX_ERRORTYPE OMXVideoDecoderAVC::PrepareConfigBuffer(VideoConfigBuffer *p) {
    ...
    ALOGW("AVC Video decoder used in Video Conferencing Mode.");
...
OMX_ERRORTYPE OMXVideoDecoderAVC::PrepareDecodeBuffer(OMX_BUFFERHEADERTYPE *buffer, buffer_retain_t *retain, VideoDecodeBuffer *p) {
    ...
    ALOGW("Received fragmented buffer.");
...
OMX_ERRORTYPE OMXVideoDecoderAVC::SetParamIntelAVCDecodeSettings(OMX_PTR pStructure) {
    ALOGW("SetParamIntelAVCDecodeSettings");
    ...
    ALOGI("Maximum width = %lu, height = %lu, dpb = %lu", p->nMaxWidth, p->nMaxHeight, p->nMaxNumberOfReferenceFrame);
    mDecodeSettings = *p;
