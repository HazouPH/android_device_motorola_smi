...
OMXVideoEncoderMPEG4::OMXVideoEncoderMPEG4() {
    ALOGV("OMXVideoEncoderMPEG4 is constructed.");
    BuildHandlerList();
...
OMXVideoEncoderMPEG4::~OMXVideoEncoderMPEG4() {
    ALOGV("OMXVideoEncoderMPEG4 is destructed.");
}
...
OMX_ERRORTYPE OMXVideoEncoderMPEG4::ProcessorProcess(
    ...
    if (!buffers[INPORT_INDEX]->nFilledLen) {
        ALOGV("%s(),%d: input buffer's nFilledLen is zero\n",  __func__, __LINE__);
        goto out;
    }
    ...
    ALOGV("inBuf.data=%x, size=%d", (unsigned)inBuf.data, inBuf.size);
    ...
    if (mFirstFrame) {
        ...
        ALOGV("output codec data size = %d", outBuf.dataSize);
...
OMX_ERRORTYPE OMXVideoEncoderMPEG4::SetParamVideoProfileLevelQuerySupported(OMX_PTR pStructure) {
    ALOGW("SetParamVideoMpeg4ProfileLevel is not supported.");
